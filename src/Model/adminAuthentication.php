...
if ( !isset($_POST['adminName'], $_POST['adminPass'], $_POST['loginNum']) ) {
...
if ($stmt->num_rows > 0) {
    $stmt->bind_result($adminID, $password, $loginNum);
    $stmt->fetch();
    // Account exists, now we verify the password.
    // Note: remember to use password_hash in your registration file to store the hashed passwords.
    if ($_POST['adminPass'] === $password & $_POST['loginNum'] === $loginNum) {
        if ($resultConfirmed->num_rows > 0) {
            ...
            $_SESSION['loginNumber'] = $loginNum;
            header("refresh:1; url=../../public/adminPortal.php");
        }
        else {
            session_regenerate_id();
            $_SESSION['adminLoggedin'] = TRUE;
            ...
            $_SESSION['AdminIDs'] = $adminID;
            $_SESSION['loginNumber'] = $loginNum;
            header("Location: ../../public/adminPortal.php");
        }
    } else {
        // Incorrect password
        echo 'Incorrect Username, Password and/or Login Number!';
    }
} else {
    // Incorrect username
    echo 'Incorrect Username, Password and/or Login Number!';
